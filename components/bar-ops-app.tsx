...
function Dashboard({ shifts, products, employees, timeEntries, tasks, shiftNotes, devMode, onNavigate }: { shifts: Shift[]; products: Product[]; employees: Employee[]; timeEntries: TimeEntry[]; tasks: OpsTask[]; shiftNotes:ShiftNote[]; devMode: boolean; onNavigate: (id: NavKey) => void }) {
  ...
  return <div className={`${dashboardStyles.dashboard} page-flow`}>
    ...
    <section className={`panel ${dashboardStyles.heroPanel}`}><PanelTitle title="Live shift board" subtitle={`${runningEntries.length} clocked in · ${late.length} late · ${upcoming.length} expected`} action={<button type="button" className="text-button" onClick={() => onNavigate("attendance")}>Open attendance <ArrowRight size={15} /></button>} />
    ...
    <section className={`panel ${dashboardStyles.summary}`}><PanelTitle title="Operational summary" subtitle="Current progress and outstanding work for today" />
    ...
    <section className={`panel quick-panel ${dashboardStyles.quickPanel}`}><PanelTitle title="Quick actions" subtitle="Jump directly into today’s work" /><div className={overviewStyles.quickGrid}>
...
function ShiftExecution({ shifts, entries, notes, onNavigate }: { shifts: Shift[]; entries: TimeEntry[]; notes: ShiftNote[]; onNavigate: (id: NavKey) => void }) {
  ...
  return <div className={`${executionStyles.workspace} page-flow`}>
    ...
    <section className={`panel ${executionStyles.board}`}><PanelTitle title="Current shift board" subtitle="Attendance, breaks and shift context" />
...
function DailyOperations({tasks,setTasks,logs,setLogs,notify,devMode,locationId}:{tasks:OpsTask[];setTasks:React.Dispatch<React.SetStateAction<OpsTask[]>>;logs:LogEntry[];setLogs:React.Dispatch<React.SetStateAction<LogEntry[]>>;notify:(s:string)=>void;devMode:boolean;locationId:string}){
 ...
 <section className={`${surfaceStyles.metrics} ${operationsStyles.summary}`} aria-label="Daily operations summary"><article><ClipboardList/><span>Opening & closing<strong>{tasks.filter(t=>t.type==="Opening"||t.type==="Closing").length} checks</strong></span></article><article><Wrench/><span>Maintenance<strong>{tasks.filter(t=>t.type==="Maintenance"&&!t.done).length} open</strong></span></article><article><NotebookPen/><span>Logbook<strong>{logs.length} entries</strong></span></article></section>
  ...
  return <div className={`${settingsStyles.workspace} page-flow`}>
    ...
    <div className={settingsStyles.layout}>
      ...
      <section className={`panel ${settingsStyles.panel}`}>
        {section==="general" && <><PanelTitle title="Organization & location" subtitle="The active workspace context used by scheduling, inventory and attendance."/><div className={`${surfaceStyles.metrics} ${settingsStyles.summary}`}><div><span>Current location</span><strong>{location?.name || "No active location"}</strong></div><div><span>Available locations</span><strong>{locations.length}</strong></div><div><span>Your role</span><strong>{userRole.replaceAll("_"," ")}</strong></div></div><p className={settingsStyles.help}>Location creation and organization identity editing are staged for a later administration release. Switch location from the top bar.</p></>}
        ...
        {section==="security" && <><PanelTitle title="Security & data" subtitle="Current production safeguards and administration status."/><div className={`${surfaceStyles.metrics} ${settingsStyles.summary}`}><div><span>Authentication</span><strong>Database sessions</strong></div><div><span>Audit trail</span><strong>Enabled</strong></div><div><span>GDPR requests</span><strong>Foundation ready</strong></div></div><p className={settingsStyles.help}>MFA enrollment, password-reset delivery, session revocation and managed backups remain in the production roadmap.</p></>}
      </section>
    </div>
  </div>;
}
