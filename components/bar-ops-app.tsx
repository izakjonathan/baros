...
function Dashboard({ shifts, products, employees, timeEntries, tasks, shiftNotes, devMode, onNavigate }: { shifts: Shift[]; products: Product[]; employees: Employee[]; timeEntries: TimeEntry[]; tasks: OpsTask[]; shiftNotes:ShiftNote[]; devMode: boolean; onNavigate: (id: NavKey) => void }) {
  ...
  return <div className={`${"dashboard"} page-flow`}>
    ...
    <section className={`panel ${"hero-panel"}`}><PanelTitle title="Live shift board" subtitle={`${runningEntries.length} clocked in · ${late.length} late · ${upcoming.length} expected`} action={<button type="button" className="text-button" onClick={() => onNavigate("attendance")}>Open attendance <ArrowRight size={15} /></button>} />
      <div className={"live-list"}>
        {liveBoard.length === 0 && <div className={"empty"}>No employees are assigned today.</div>}
        {liveBoard.map(({ shift, status, tone, detail }) => <button type="button" className={"live-row"} key={shift.id} onClick={() => onNavigate("attendance")}>
          <div className="avatar">{shift.initials}</div><div className={"live-person"}><strong>{shift.employee}</strong><small>{shift.role} · {shift.start}–{shift.end}</small></div><div className={"live-status"} data-tone={tone}><span>{status}</span><small>{detail}</small></div><ChevronRight size={18}/>
        </button>)}
      </div>
    </section>
    <div className={`${"content-grid"}`}>
      <section className={`panel today-panel ${"panel"}`}><PanelTitle title="Today’s timeline" subtitle={`${assignedToday.length} assigned · ${openToday.length} open`} action={<button className="text-button" onClick={() => onNavigate("schedule")}>Open shift plan <ArrowRight size={15} /></button>} />
        <div className="timeline">
          {timeline.length === 0 && <div className={"empty"}>No shifts scheduled today.</div>}
          {timeline.map((shift) => <button type="button" className={`timeline-row ${"timeline-row"}`} key={shift.id} onClick={() => onNavigate("schedule")}><time>{shift.start}</time><div className={`avatar ${shift.isOpen ? "sand" : ""}`}>{shift.isOpen ? "+" : shift.initials}</div><div className="grow"><strong>{shift.employee}</strong><span>{shift.role}{shift.availabilityConflict ? " · availability conflict" : ""}</span></div><span className="shift-time">{shift.start}–{shift.end}</span><ChevronRight size={18}/></button>)}
        </div>
      </section>
      <section className={`panel attention-panel ${"panel"} ${"attention-panel"}`}><PanelTitle title="Attention needed" subtitle="Prioritised operational actions" />
    ...
    <section className={`panel ${"summary"}`}><PanelTitle title="Operational summary" subtitle="Current progress and outstanding work for today" />
    ...
    {shiftNotes.length>0&&<section className={`panel shift-notes-panel ${"panel"}`}><PanelTitle title="Latest shift notes" subtitle="Incidents, equipment, stock and handover notes from the team" action={<button type="button" className="text-button" onClick={()=>onNavigate("schedule")}>Open schedule <ArrowRight size={15}/></button>}/><div className={"notesList"}>{shiftNotes.slice(0,6).map(note=><article key={note.id}><span className={"note-category"}>{note.category}</span><div><strong>{note.author} · {note.role}</strong><p>{note.note}</p><small>{new Date(note.createdAt).toLocaleString("en-GB",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})}</small></div></article>)}</div></section>}
    <section className={`panel quick-panel ${"quick-panel"}`}><PanelTitle title="Quick actions" subtitle="Jump directly into today’s work" /><div className={"quick-grid"}>
...
function ShiftExecution({ shifts, entries, notes, onNavigate }: { shifts: Shift[]; entries: TimeEntry[]; notes: ShiftNote[]; onNavigate: (id: NavKey) => void }) {
  ...
  return <div className={`${"execution-workspace"} page-flow`}>
    <div className={""}><PageHeader eyebrow="Live operations" title="Shift execution" subtitle="Run the current shift from one operational view" action={<button type="button" className="primary action-button" onClick={()=>onNavigate("attendance")}><Timer size={16}/>Open attendance</button>} /></div>
    <section className={`${"metrics"}`}>
...
function Metric({ icon: Icon, label, value, detail, trend, warning }: { icon: typeof Users; label: string; value: string; detail: string; trend: string; warning?: boolean }) {
  return <div className={"metric"}><span className={"metric-label"}>{label}</span><strong>{value}</strong><small>{detail}</small><div className={`${"metric-trend"} ${warning ? "warning" : ""}`}>{warning ? <AlertTriangle size={13} /> : <span aria-hidden="true" className={"status-dot"} />}{trend}</div></div>
}
...
function Quick({ icon: Icon, label, detail, onClick }: { icon: typeof CalendarDays; label: string; detail: string; onClick: () => void }) { return <button className={"quick-action"} onClick={onClick}><span className={"quick-icon"}><Icon size={19} /></span><div><strong>{label}</strong><small>{detail}</small></div><ArrowRight size={17} /></button> }
...
function Schedule({ shifts, setShifts, employees, onNewShift, onEditShift, notify, currentWeekOffset, setCurrentWeekOffset, devMode, selectedLocationId, persist }: { shifts: Shift[]; setShifts: React.Dispatch<React.SetStateAction<Shift[]>>; employees: Employee[]; onNewShift: (date?: string) => void; onEditShift: (shift: Shift) => void; notify: (s: string) => void; currentWeekOffset: number; setCurrentWeekOffset: React.Dispatch<React.SetStateAction<number>>; devMode: boolean; selectedLocationId: string; persist: (path:string, options:RequestInit) => Promise<any> }) {
  ...
  return <div className={`${"schedule-workspace"} page-flow`}>
    <div className={"schedule-header"}>
      <div className={"headerCopy"}><p className="eyebrow">{rangeLabel}</p><h1>Shift plan</h1></div>
      <div className={"header-actions"}><button className="secondary compact-action" onClick={copyPreviousWeek} disabled={viewMode !== "week"}><Copy size={15} /><span>Copy previous week</span></button><button className={`${"top-add-shift"} primary compact-action`} onClick={() => onNewShift(displayDays[0]?.iso)}><Plus size={16} /><span>Add shift</span></button></div>
    </div>
    <section className={`${"schedule-toolbar"} ${viewMode === "custom" ? "custom-mode" : ""}`}>
      <div className={"period-controls"}><button onClick={() => movePeriod(-1)} aria-label={`Previous ${viewMode}`}><ChevronLeft size={17}/></button><strong>{compactPeriodLabel}</strong><button onClick={() => movePeriod(1)} aria-label={`Next ${viewMode}`}><ChevronRight size={17}/></button></div>
      <label className={"view-select"}><span className={"view-label"} aria-hidden="true">{viewMode === "custom" ? "Period" : viewMode === "month" ? "Month" : "Week"}</span><select aria-label="Schedule view" value={viewMode} onChange={(event) => setViewMode(event.target.value as "week" | "month" | "custom")}><option value="week">Week</option><option value="month">Month</option><option value="custom">Period</option></select></label>
...
function ShiftCard({ shift, conflict, onOpen, onDragStart }: { shift: Shift; conflict?: boolean; onOpen: () => void; onDragStart?: (event: React.DragEvent<HTMLButtonElement>) => void }) {
  ...
  return <button type="button" draggable onDragStart={onDragStart} className={"shift-card"} data-role={shift.role} data-draft={shift.status === "Draft"} data-conflict={Boolean(conflict)} onClick={onOpen} aria-label={`Open ${shift.isOpen ? "available" : shift.employee} shift ${shift.start} to ${shift.end}${overnight ? " next day" : ""}`}>
    <div className={"shift-card-top"}><span>{shift.start}–{shift.end}{overnight ? " +1" : ""}</span><ChevronRight size={14} /></div>
    <strong>{displayName}</strong>
    <small><span>{shift.role}</span>{overnight ? <span className={"overnight-label"}> · Overnight</span> : null}</small>
  ...
  return <div className={"workspace"}>
    <header className={"attendance-hero"}>
      ...
      <div className={"actions"}>
        {canManagePayroll && <button className={`${"control"} ${periodLocked ? "" : "outline"}`} onClick={()=>{setPeriodLocked(v=>!v);notify(periodLocked?"Payroll period unlocked":"Payroll period locked for export")}}>{periodLocked?<><LockKeyhole size={17}/>Locked</>:<><UnlockKeyhole size={17}/>Lock period</>}</button>}
        <button className={`${"control"} ${""}`} onClick={approveAllVisible} disabled={periodLocked||!visible.some(e=>e.status==="Pending")}><CheckCheck size={17}/>Approve visible</button>
        {canExportPayroll && <button className={`${"control"} ${""}`} onClick={exportApproved} disabled={!approved.length||!periodLocked}><FileDown size={17}/>Export</button>}
      </div>
    </header>

    <section className={"attendance-filters"} aria-label="Timesheet filters">
      <div className={"period-fields"}>
        <label>From<span className={"date-control"}><span aria-hidden="true">{formatAttendanceDate(fromDate)}</span><input aria-label="Payroll period from date" type="date" value={fromDate} onChange={e=>setFromDate(e.target.value)}/></span></label>
        <label>To<span className={"date-control"}><span aria-hidden="true">{formatAttendanceDate(toDate)}</span><input aria-label="Payroll period to date" type="date" value={toDate} min={fromDate} onChange={e=>setToDate(e.target.value)}/></span></label>
      </div>
      <div className={"filter-fields"}><label>Employee<select value={employeeFilter} onChange={e=>setEmployeeFilter(e.target.value)}><option>All employees</option>{employees.map(e=><option key={e.name}>{e.name}</option>)}</select></label><label>Status<select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option>Needs review</option><option>All</option><option>Pending</option><option>Approved</option><option>Rejected</option><option>Running</option></select></label></div>
      <div className={"filter-footer"}><span role="status" aria-live="polite">{visible.length} timesheet{visible.length===1?"":"s"}</span><button type="button" onClick={()=>{setEmployeeFilter("All employees");setStatusFilter("Needs review")}} className={`${"control"} ${"solid"}`} disabled={employeeFilter==="All employees"&&statusFilter==="Needs review"}>Reset</button></div>
    </section>

    <section className={"records"}>
      <div className={`${"section-header"} ${"section-header"}`}><div><h2>Timesheets</h2><p>Review each record and act on exceptions.</p></div><span>{periodLocked?"Period locked":"Open period"}</span></div>
      <div className={"record-list"}>{visible.map(e=>{const actual=e.clockOut?workedHours(e):0;const variance=actual-e.scheduledHours;const exception=e.status!=="Running"&&(Math.abs(variance)>=.5||e.breakMinutes===0||e.edited);return <article className={`${"record"} ${exception?"record-exception":""}`} key={e.id}>
        <header><div><strong>{e.employee}</strong><span>{new Date(`${e.date}T12:00:00`).toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})}</span></div><i className={`status status-${e.status.toLowerCase()}`}>{e.status}</i></header>
        <div className={"time-line"}><b>{e.clockIn}–{e.clockOut||"Now"}</b><span>{e.clockOut?`${actual.toFixed(2)}h worked`:"Clocked in"}</span></div>
        <div className={"record-stats"}><span>Scheduled<b>{e.scheduledHours.toFixed(1)}h</b></span><span>Break<b>{e.breakMinutes}m</b></span><span>Variance<b className={Math.abs(variance)>=.5?"alert-text":""}>{e.clockOut?`${variance>=0?"+":""}${variance.toFixed(2)}h`:"—"}</b></span></div>
        {e.edited&&<small className={""}>Manager corrected</small>}
        <div className={"record-actions"}>{e.status==="Running"&&<button type="button" className={`${"control"} ${"outline"}`} onClick={()=>void toggleBreak(e)}>{e.onBreak?"End break":"Start break"}</button>}{e.status==="Pending"&&<><button className={`${"control"} ${"outline"}`} aria-label={`Edit ${e.employee} timesheet`} onClick={()=>onEdit(e)}><Pencil size={16}/></button><button className={`${"control"} ${"outline"}`} aria-label={`Reject ${e.employee} timesheet`} onClick={()=>rejectTimesheet(e.id)}><Ban size={16}/></button><button className={`${"control"} ${"solid"} ${"primary-action"}`} onClick={()=>approveTimesheet(e.id)}>Approve</button></>}{e.status==="Approved"&&<button className={`${"control"} ${"outline"}`} onClick={()=>reopenTimesheet(e.id)}><RotateCcw size={15}/>Reopen</button>}{e.status==="Rejected"&&<button className={`${"control"} ${"outline"}`} onClick={()=>reopenTimesheet(e.id)}><RotateCcw size={15}/>Return to review</button>}</div>
      </article>})}{!visible.length&&<div className={`${"empty"} ${"empty"}`}><strong>No timesheets found</strong><span>Change the filters or date range to widen the results.</span></div>}</div>
    </section>

    <section className={`${"metrics"} ${"summary"}`} aria-label="Payroll summary">
      ...
      <article className={pending?"":""}><span>Awaiting</span><strong>{pending}</strong></article>
      <article className={exceptions.length?"":""} aria-label="Exceptions: Variance, no break, or edited"><span>Exceptions</span><strong>{exceptions.length}</strong></article>
    </section>

    <section className={"preview"}>
      <div className={`${"section-header"} ${"section-header"}`}><div><h2>Payroll preview</h2><p>Approved hours for the selected period.</p></div></div>
      <div className={"preview-list"}>{payrollRows.length?payrollRows.map(({emp,scheduledEmp,approvedEntries,workedEmp})=><article key={emp.name}><div><span className={"avatar"}>{emp.initials}</span><p><strong>{emp.name}</strong><small>{emp.role}</small></p></div><dl><div><dt>Scheduled</dt><dd>{scheduledEmp.toFixed(1)}h</dd></div><div><dt>Approved</dt><dd>{workedEmp.toFixed(2)}h</dd></div></dl><small>{approvedEntries.length} approved record{approvedEntries.length===1?"":"s"}</small></article>):<div className={`${"empty"} ${"empty"}`}><strong>No payroll rows yet</strong><span>Approved timesheets appear here.</span></div>}</div>
    </section>

    <section className={"history"}>
      <div className={`${"section-header"} ${"section-header"}`}><div><h2>Export history</h2><p>Files generated in this session.</p></div><History size={18}/></div>
...
function Inventory({ products, setProducts, onNewProduct, onEditProduct, onStockCount, adjustments, setAdjustments, notify, devMode, selectedLocationId, persist }: { products: Product[]; setProducts: React.Dispatch<React.SetStateAction<Product[]>>; onNewProduct: () => void; onEditProduct: (product:Product)=>void; onStockCount:()=>void; adjustments:StockAdjustment[]; setAdjustments:React.Dispatch<React.SetStateAction<StockAdjustment[]>>; notify: (s: string) => void; devMode:boolean; selectedLocationId:string; persist:(path:string,options:RequestInit)=>Promise<any> }) {
  ...
  return <div className={`${"workspace"} page-flow`}>
    <PageHeader title="Inventory" subtitle="Stock, par levels and product controls." action={<div className={`header-actions ${"actions"}`}><button className={`secondary ${"control"}`} onClick={onStockCount}><ClipboardList size={17} /> Stock count</button><button className={`primary ${"control"}`} onClick={onNewProduct}><Plus size={18} /> Add product</button></div>} />
    <section className={`${"metrics"} ${"summary"}`}><div><span>Stock value</span><strong>{money(value)}</strong></div><div><span>Products</span><strong>{products.filter(p=>p.active!==false).length}</strong></div><div><span>Below par</span><strong>{products.filter((p) => p.active!==false&&p.stock < p.par).length}</strong></div><div><span>Suppliers</span><strong>{new Set(products.map(p=>p.supplier)).size}</strong></div></section>
    <section className={""}><div className={`${"toolbar"} ${"inventory-toolbar"}`}><div className={"search"}><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search inventory" aria-label="Search inventory" /></div><select value={category} onChange={e=>setCategory(e.target.value)} aria-label="Filter inventory category">{categories.map(c=><option key={c}>{c}</option>)}</select><button className={`${"control"} ${""} ${onlyLow ? "selected" : ""}`} aria-pressed={onlyLow} onClick={() => setOnlyLow(!onlyLow)}><AlertTriangle size={16} /> Below par</button>{(query||category!=="All categories"||onlyLow)&&<button type="button" className={`${"control"} ${""}`} onClick={()=>{setQuery("");setCategory("All categories");setOnlyLow(false)}}>Clear</button>}<span className={"result-count"} role="status" aria-live="polite">{filtered.length} product{filtered.length===1?"":"s"}</span></div>
      <div className={"product-list"}>{filtered.map((product) => { const suggested = Math.max(0, product.par - product.stock); return <article className={`${"card"} ${"product-card"}`} key={product.id}><header className={"card-header"}><div><p>{product.category}</p><h2>{product.name}</h2><small>{product.supplier} · {product.sku||"No SKU"}</small></div><button className={"edit-button"} onClick={()=>onEditProduct(product)} aria-label={`Edit ${product.name}`}><Pencil size={15}/></button></header><div className={"stock-row"}><div><span>In stock</span><div className={"stock-control"}><button aria-label={`Remove one ${product.name}`} onClick={() => adjust(product,-1)}>−</button><strong className={product.stock < product.par ? "low" : ""}>{product.stock}</strong><button aria-label={`Add one ${product.name}`} onClick={() => adjust(product,1)}>+</button></div><small>{product.unit}</small></div><div><span>Par</span><strong>{product.par}</strong><small>Reorder at {product.reorderLevel??Math.max(0,product.par-2)}</small></div><div><span>Suggested</span>{suggested > 0 ? <strong className={"suggestion"}>+{suggested}</strong> : <strong className={"ok"}><Check size={14} /> OK</strong>}<small>{money(product.price)} / {product.unit}</small></div></div></article>})}{!filtered.length&&<div className={`${"empty"} ${"empty"}`}><strong>No products found</strong><span>Change the search or filters.</span></div>}</div>
    </section>
    <section className={`${"card"} ${"history"}`}><div className={"card-header"}><div><h2>Adjustment history</h2><p>Recent manual stock changes.</p></div><History size={18}/></div>{adjustments.length?<div>{adjustments.slice(0,8).map(a=><article key={a.id}><span className={a.delta>0?"positive":"negative"}>{a.delta>0?"+":""}{a.delta}</span><span><b>{a.productName}</b><small>{a.reason}</small></span><time>{a.createdAt}</time></article>)}</div>:<p>No manual stock adjustments yet.</p>}</section>
  </div>
}
...
function Orders({ products, setProducts, onNewOrder, notify }: { products:Product[]; setProducts:React.Dispatch<React.SetStateAction<Product[]>>; onNewOrder: () => void; notify: (s: string) => void }) {
  ...
  return <div className={`${"workspace"} page-flow`}><PageHeader title="Purchase orders" subtitle="Replenishment, deliveries and order status." action={<button className={`${"control"} ${"solid"} ${"team-add-button"}`} onClick={onNewOrder}><Plus size={18} /> New order</button>} />
    {suggestions.length>0&&<section className={"suggestion"}><div><Sparkles size={19}/><div><p>Suggested replenishment</p><strong>{suggestions.length} products below par</strong><small>{suggestions.reduce((n,p)=>n+p.qty,0)} units suggested</small></div></div><button className={"control"} onClick={()=>notify("Suggested order grouped by supplier and saved as draft")}>Create draft</button></section>}
    <section className={"delivery"}><div><Truck size={20}/><div><p>Next delivery</p><strong>Nordic Drinks</strong><small>Tomorrow · 08:00–11:00</small></div></div><button className={"control"} onClick={() => receive("PO-1048")}>{received.includes("PO-1048")?<><Check size={16}/>Received</>:"Receive"}</button></section>
    <section className={""}><div className={`${"toolbar"} ${"toolbar"}`}><div className={"search"}><Search size={17} /><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search orders" aria-label="Search orders" /></div><label className={"filter"}><span className="sr-only">Filter order status</span><select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="ALL">All statuses</option><option value="DRAFT">Draft</option><option value="CONFIRMED">Confirmed</option><option value="DELIVERED">Delivered</option></select><ChevronDown size={15}/></label>{(query||statusFilter!=="ALL")&&<button type="button" className={`${"control"} ${""}`} onClick={()=>{setQuery("");setStatusFilter("ALL")}}>Clear</button>}<span className={"result-count"} role="status" aria-live="polite">{visibleOrders.length} order{visibleOrders.length===1?"":"s"}</span></div><div className={"order-list"}>{visibleOrders.length?visibleOrders.map((order) => {const currentStatus=received.includes(order.id)?"Received":order.status;return <article className={`${"card"} ${"order-card"}`} key={order.id}><header className={"card-header"}><div><p>{order.id}</p><h2>{order.supplier}</h2></div><span data-status={currentStatus.toLowerCase()}>{currentStatus}</span></header><dl><div><dt>Items</dt><dd>{order.items}</dd></div><div><dt>Delivery</dt><dd>{order.delivery}</dd></div><div><dt>Amount</dt><dd>{money(order.amount)}</dd></div></dl>{order.status!=="Delivered"&&!received.includes(order.id)&&<button className={`${"control"} ${"solid"} ${""}`} onClick={()=>receive(order.id)}><CheckCircle2 size={16}/> Receive order</button>}</article>}):<div className={`${"empty"} ${"empty"}`}><strong>No orders found</strong><span>Change the search or status filter.</span></div>}</div></section>
  </div>
}
...
function DailyOperations({tasks,setTasks,logs,setLogs,notify,devMode,locationId}:{tasks:OpsTask[];setTasks:React.Dispatch<React.SetStateAction<OpsTask[]>>;logs:LogEntry[];setLogs:React.Dispatch<React.SetStateAction<LogEntry[]>>;notify:(s:string)=>void;devMode:boolean;locationId:string}){
 ...
 return <div className={`${"workspace"} page-flow`}>
 <div className={"operations-header"}><PageHeader title="Daily operations" subtitle="Opening, closing, handovers and maintenance in one live manager workspace." action={<div className={"header-actions"}><input className={""} type="date" value={serviceDate} max={new Date().toISOString().slice(0,10)} onChange={e=>setServiceDate(e.target.value)} aria-label="Checklist date"/><span className={`connection-pill dev ${""}`}>{complete}/{tasks.length} complete</span></div>}/></div>
 <section className={`${"metrics"} ${"summary"}`} aria-label="Daily operations summary"><article><ClipboardList/><span>Opening & closing<strong>{tasks.filter(t=>t.type==="Opening"||t.type==="Closing").length} checks</strong></span></article><article><Wrench/><span>Maintenance<strong>{tasks.filter(t=>t.type==="Maintenance"&&!t.done).length} open</strong></span></article><article><NotebookPen/><span>Logbook<strong>{logs.length} entries</strong></span></article></section>
 <div className={"operations-layout"}><section className={`panel ${"panel"} ${"checklist"}`}><PanelTitle title="Today’s checklist" subtitle="Completion is shared with every manager at this location."/>{isToday&&<div className={"presets"}><button type="button" className="secondary" onClick={()=>addPreset("Opening")}><Plus size={15}/>Opening checklist</button><button type="button" className="secondary" onClick={()=>addPreset("Closing")}><Plus size={15}/>Closing checklist</button></div>}<div className={"task-list"}>{tasks.map(t=><article key={t.id} className={`${"task"} ${t.done?"task-done":""}`}><button type="button" className="task-check" disabled={!isToday||savingId===t.id} onClick={()=>toggleTask(t)}>{t.done?<Check size={16}/>:null}</button><span><b>{t.title}</b><small>{t.type} · {t.owner} · {t.due}</small></span>{isToday&&<span className={"row-actions"}><button type="button" className="icon-button" onClick={()=>editTaskMeta(t)} aria-label={`Edit responsibility for ${t.title}`}><Pencil size={15}/></button><button type="button" className="icon-button" onClick={()=>removeTask(t)} aria-label={`Remove ${t.title}`}><Trash2 size={15}/></button></span>}</article>)}</div>{isToday&&<div className={"create"}><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Add an operational task"/><select value={type} onChange={e=>setType(e.target.value as OpsTask["type"])}><option>Task</option><option>Opening</option><option>Closing</option><option>Maintenance</option></select><button type="button" className="primary" onClick={addTask}><Plus size={16}/>Add</button></div>}</section>
 <section className={`panel ${"panel"} ${"logbook"}`}><PanelTitle title="Manager logbook" subtitle="Permanent shift handovers and important operational context."/><div className={"compose"}><textarea value={logText} onChange={e=>setLogText(e.target.value)} placeholder="What does the next manager need to know?"/><button type="button" className="primary" onClick={addLog}><Send size={16}/>Save handover</button></div><div className={"log-list"}>{logs.map(l=><article className={"log-entry"} key={l.id}><div><b>{l.title}</b><small>{l.author} · {l.createdAt}</small></div><p>{l.body}</p></article>)}</div></section></div></div>
}
...
function Team({ employees, shifts, devMode, canManage, onAdd, onEdit, onInvite, onRevoke }: { employees: Employee[]; shifts: Shift[]; devMode:boolean; canManage:boolean; onAdd: () => void; onEdit: (employee: Employee) => void; onInvite:(employee:Employee)=>void; onRevoke:(employee:Employee)=>void }) {
  ...
  return <div className={`${"workspace"} page-flow`}>
    <PageHeader eyebrow="People operations" title="Team" subtitle="People, access and upcoming hours." action={canManage ? <button className={`team-add-button ${"control"} ${"solid"} ${"team-add-button"}`} onClick={onAdd}><UserRoundPlus size={17}/>Add employee</button> : <span className="connection-pill">Read only</span>}/>
    <section className={`${"metrics"} ${"summary"}`} aria-label="Team summary">
    ...
    <div className={"team-toolbar"}>
      <div className={"search"}><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search team" aria-label="Search team"/></div>
      ...
      {(query||status!=="ALL")&&<button type="button" className={`secondary compact ${"control"} ${"clear-button"}`} onClick={()=>{setQuery("");setStatus("ALL")}}>Clear</button>}
      <span className={"result-count"} role="status" aria-live="polite">{visibleEmployees.length} employee{visibleEmployees.length===1?"":"s"}</span>
    </div>
    <section className={"team-grid"}>
      {visibleEmployees.map((person) => {
        ...
        return <article className={`${"card"} ${!person.active ? "inactive" : ""}`} key={person.id||person.name}>
          <header className={`${"card-header"} ${"card-header"}`}>
            <div className={"identity"}>
              <div className={"avatar"} aria-hidden="true">{person.initials}</div>
              <div><p className={"role"}>{person.role}</p><h2>{person.name}</h2></div>
            </div>
            <span className={person.active?"active-status":"inactive-status"}>{person.active?"Active":"Inactive"}</span>
          </header>
          ...
          <div className={"portal-row"}><span className={"indicator"} data-state={person.portalStatus}/><span>{portalLabel}</span></div>
          {canManage && <footer className={"actions"}>
            <button className={`${"control"} ${"outline"} ${"outline-button"}`} onClick={() => onEdit(person)}>Edit</button>
            <button className={`${"control"} ${"solid"} ${"filled-button"}`} disabled={devMode||!person.email||person.portalStatus==="ACTIVE"} onClick={()=>onInvite(person)}>{person.portalStatus==="INVITED"?"Resend":"Invite"}</button>
            {person.portalStatus==="INVITED"&&<button className={`${"control"} ${"outline"} ${"outline-button"}`} disabled={devMode} onClick={()=>onRevoke(person)}>Revoke</button>}
          </footer>}
          {devMode&&<small className={"note"}>Connect PostgreSQL to create real employee logins.</small>}
        </article>
      })}
      {!visibleEmployees.length&&<div className={`${"empty"} ${"empty"}`}>No employees match the current search and filters.</div>}
    </section>
  </div>
}
...
function SettingsWorkspace({ locations, selectedLocationId, userRole, devMode, notify }: { locations: Location[]; selectedLocationId: string; userRole: string; devMode: boolean; notify: (message:string)=>void }) {
  ...
  return <div className={`${"workspace"} page-flow`}>
    <div className={""}><PageHeader eyebrow="Workspace configuration" title="Settings" /></div>
    <div className={"settings-layout"}>
      ...
      <section className={`panel ${"panel"}`}>
        {section==="general" && <><PanelTitle title="Organization & location" subtitle="The active workspace context used by scheduling, inventory and attendance."/><div className={`${"metrics"} ${"summary"}`}><div><span>Current location</span><strong>{location?.name || "No active location"}</strong></div><div><span>Available locations</span><strong>{locations.length}</strong></div><div><span>Your role</span><strong>{userRole.replaceAll("_"," ")}</strong></div></div><p className={"muted-note"}>Location creation and organization identity editing are staged for a later administration release. Switch location from the top bar.</p></>}
        {section==="time" && <><PanelTitle title="Time clock" subtitle="Control mobile and kiosk attendance for the selected location."/>{loading?<div className={"empty"}>Loading settings…</div>:<div className={"form-grid"}>
          <label className={"toggle"}><span><strong>Mobile clock-in</strong><small>Allow linked employees to clock in from their portal.</small></span><input type="checkbox" checked={clock.allowMobileClock} onChange={e=>setClock({...clock,allowMobileClock:e.target.checked})}/></label>
          <label className={"toggle"}><span><strong>Kiosk clock-in</strong><small>Allow PIN-based clocking from a shared device.</small></span><input type="checkbox" checked={clock.allowKioskClock} onChange={e=>setClock({...clock,allowKioskClock:e.target.checked})}/></label>
          <label className={"toggle"}><span><strong>Unscheduled clock-in</strong><small>Permit clock-in when no nearby published shift exists.</small></span><input type="checkbox" checked={clock.allowUnscheduledClock} onChange={e=>setClock({...clock,allowUnscheduledClock:e.target.checked})}/></label>
          <label className={"toggle"}><span><strong>Require location check</strong><small>Require location verification when geofencing is configured.</small></span><input type="checkbox" checked={clock.requireLocationCheck} onChange={e=>setClock({...clock,requireLocationCheck:e.target.checked})}/></label>
          <div className={"fields"}><label>Early clock-in window<input type="number" min="0" max="240" value={clock.earlyClockInMinutes} onChange={e=>setClock({...clock,earlyClockInMinutes:Number(e.target.value)})}/><small>Minutes before the shift</small></label><label>Missed clock-out threshold<input type="number" min="0" max="720" value={clock.lateClockOutMinutes} onChange={e=>setClock({...clock,lateClockOutMinutes:Number(e.target.value)})}/><small>Minutes after scheduled end</small></label><label>Rounding<select value={clock.roundingMinutes} onChange={e=>setClock({...clock,roundingMinutes:Number(e.target.value)})}><option value="0">No rounding</option><option value="5">5 minutes</option><option value="6">6 minutes</option><option value="10">10 minutes</option><option value="15">15 minutes</option></select></label><label>Auto-approval tolerance<input type="number" min="0" max="240" value={clock.autoApproveWithinMinutes} onChange={e=>setClock({...clock,autoApproveWithinMinutes:e.target.value===""?"":Number(e.target.value)})}/><small>Leave blank for manager approval</small></label></div>
          <div className={"actions"}><button className={`primary ${"control"}`} disabled={!canManage||saving} onClick={saveClock}><Save size={17}/>{saving?"Saving…":"Save settings"}</button><a className={`secondary settings-link ${"control"}`} href="/employee/hours"><Clock3 size={17}/>Open my time clock</a>{!canManage&&<small>Owner, Admin or Manager permission is required to change settings.</small>}</div>
        </div>}</>}
        {section==="security" && <><PanelTitle title="Security & data" subtitle="Current production safeguards and administration status."/><div className={`${"metrics"} ${"summary"}`}><div><span>Authentication</span><strong>Database sessions</strong></div><div><span>Audit trail</span><strong>Enabled</strong></div><div><span>GDPR requests</span><strong>Foundation ready</strong></div></div><p className={"muted-note"}>MFA enrollment, password-reset delivery, session revocation and managed backups remain in the production roadmap.</p></>}
      </section>
    </div>
  </div>;
}
