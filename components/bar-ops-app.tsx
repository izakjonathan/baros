...
export function BarOpsApp({ userName, userRole, devMode }: { userName: string; userRole: string; devMode: boolean }) {
  ...
  return (
    <div className="app-frame">
      ...
      {editingProduct && <ProductDialog product={editingProduct} onClose={() => setEditingProduct(null)} onSave={(product) => { setProducts(current => current.map(p => p.id === product.id ? product : p)); persist("/api/products",{method:"PATCH",body:JSON.stringify(product)}).catch(e=>notify(e.message)); setEditingProduct(null); notify("Product updated"); }} />}
