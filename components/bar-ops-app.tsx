  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <div className="side-bottom">
        ...
        {devMode && <DevRoleSwitcher currentRole={userRole} />}<button type="button" onClick={async()=>{await fetch("/api/auth/logout",{method:"POST"});window.location.href="/login"}}><LogOut size={19}/><span>Sign out</span></button><div className="profile"><div className="avatar dark">{userName.split(" ").map(part => part[0]).join("").slice(0,2)}</div><div><strong>{userName}</strong><span>{userRole.replace("_", " ").toLowerCase()}</span></div><ChevronDown size={16} /></div>
      </div>
    </aside>
  </>
}
...
function Schedule({ shifts, setShifts, employees, onNewShift, onEditShift, notify, currentWeekOffset, setCurrentWeekOffset, devMode, selectedLocationId, persist }: { shifts: Shift[]; setShifts: React.Dispatch<React.SetStateAction<Shift[]>>; employees: Employee[]; onNewShift: (date?: string) => void; onEditShift: (shift: Shift) => void; notify: (s: string) => void; currentWeekOffset: number; setCurrentWeekOffset: React.Dispatch<React.SetStateAction<number>>; devMode: boolean; selectedLocationId: string; persist: (path:string, options:RequestInit) => Promise<any> }) {
  ...
  async function copyPreviousPeriod() {
    if (viewMode === "custom") { notify("Copy is available in Week and Month views"); return; }
    const source = viewMode === "week"
      ? shifts.filter((shift) => (shift.weekOffset ?? 0) === currentWeekOffset - 1)
      : shifts.filter((shift) => { const date = new Date(`${canonicalShiftDate(shift)}T12:00:00`); const previous = new Date(monthAnchor.getFullYear(), monthAnchor.getMonth() - 1, 1, 12); return date.getFullYear() === previous.getFullYear() && date.getMonth() === previous.getMonth(); });
    if (!source.length) { notify(`The previous ${viewMode === "week" ? "week" : "month"} has no shifts to copy`); return; }
    ...
    const copies = source.map((shift) => { const sourceDate = new Date(`${canonicalShiftDate(shift)}T12:00:00`); if (viewMode === "week") sourceDate.setDate(sourceDate.getDate() + 7); else sourceDate.setMonth(sourceDate.getMonth() + 1); const date = toIsoDate(sourceDate); const position = shiftPositionFromDate(date); return { ...shift, id: crypto.randomUUID(), date, day: position.day, weekOffset: position.weekOffset, status: "Draft" as const, recurrenceLabel: undefined }; });
    try {
...
function ShiftDialog({ onClose, onSave, currentWeekOffset, initialDate, employees, locations, selectedLocationId }: { onClose: () => void; onSave: (shifts: Shift[]) => void; currentWeekOffset: number; initialDate?: string; employees: Employee[]; locations: Location[]; selectedLocationId: string }) {
  ...
  function save() {
    ...
    const safeCount = Math.max(1, Math.min(Number.isFinite(parsedCount) ? parsedCount : 1, frequency === "daily" ? 31 : 52));
  ...
  return <Modal title="Add shift" subtitle="Create one shift or a repeating series." onClose={onClose}>
    ...
    {repeat && <div className="repeat-panel"><div className="frequency-toggle"><button className={frequency === "daily" ? "selected" : ""} onClick={() => setFrequency("daily")}>Daily</button><button className={frequency === "weekly" ? "selected" : ""} onClick={() => setFrequency("weekly")}>Weekly</button></div>
      ...
      <label className="repeat-count">Repeat for <input type="number" min="1" max={frequency === "daily" ? 31 : 52} value={count} onChange={(e) => setCount(e.target.value.replace(/\D/g, ""))} onBlur={() => { if (!count) setCount("1"); }} inputMode="numeric"/><span>{frequency === "daily" ? "days" : "weeks"}</span></label>
    </div>}
...
function StockCountDialog({products,onClose,onSave}:{products:Product[];onClose:()=>void;onSave:(counts:Record<string,number>)=>void}){
 const [countInputs,setCountInputs]=useState<Record<string,string>>(()=>Object.fromEntries(products.filter(p=>p.active!==false).map(p=>[p.id,String(p.stock)]))); const [showExpected,setShowExpected]=useState(false); const numericCounts=Object.fromEntries(Object.entries(countInputs).map(([id,value])=>[id,value===""?0:Number(value)])); const variances=products.filter(p=>p.active!==false).map(p=>({p,variance:(numericCounts[p.id]??0)-p.stock}));
 return <Modal title="Stock count" subtitle="Enter actual quantities, review variance, then approve the count." onClose={onClose}><div className="count-toolbar"><label><input type="checkbox" checked={showExpected} onChange={e=>setShowExpected(e.target.checked)}/> Show expected stock</label><strong>{variances.filter(x=>x.variance!==0).length} variances</strong></div><div className="stock-count-list">{variances.map(({p,variance})=><label key={p.id}><span><b>{p.name}</b><small>{p.category} · {p.unit}{showExpected?` · expected ${p.stock}`:""}</small></span><input type="number" min="0" inputMode="decimal" value={countInputs[p.id]??""} onChange={e=>setCountInputs(cur=>({...cur,[p.id]:e.target.value}))} onBlur={()=>setCountInputs(cur=>({...cur,[p.id]:cur[p.id]===""?"0":cur[p.id]}))}/><i className={variance===0?"count-ok":variance>0?"count-over":"count-short"}>{variance===0?"Match":`${variance>0?"+":""}${variance}`}</i></label>)}</div><ModalActions onClose={onClose} onSave={()=>onSave(numericCounts)} label="Approve stock count"/></Modal>
}
