...
function Schedule({ shifts, setShifts, employees, onNewShift, onEditShift, notify, currentWeekOffset, setCurrentWeekOffset, devMode, selectedLocationId, persist }: { shifts: Shift[]; setShifts: React.Dispatch<React.SetStateAction<Shift[]>>; employees: Employee[]; onNewShift: (date?: string) => void; onEditShift: (shift: Shift) => void; notify: (s: string) => void; currentWeekOffset: number; setCurrentWeekOffset: React.Dispatch<React.SetStateAction<number>>; devMode: boolean; selectedLocationId: string; persist: (path:string, options:RequestInit) => Promise<any> }) {
  ...
  return <>
    ...
    <section className="schedule-toolbar compact-schedule-toolbar">
      ...
      <div className="schedule-toolbar-right"><div className="view-toggle" aria-label="Schedule view"><button className={viewMode === "week" ? "selected" : ""} onClick={() => setViewMode("week")}>Week</button><button className={viewMode === "month" ? "selected" : ""} onClick={() => setViewMode("month")}>Month</button><button className={viewMode === "custom" ? "selected" : ""} onClick={() => setViewMode("custom")}>Custom</button></div>{viewMode==="custom"&&<div className="custom-range"><input type="date" value={customFrom} onChange={e=>setCustomFrom(e.target.value)}/><span>–</span><input type="date" value={customTo} min={customFrom} onChange={e=>setCustomTo(e.target.value)}/></div>}<span className="schedule-counts"><b>{visibleShifts.length}</b> shifts · <b>{drafts}</b> drafts{conflicts.size ? <> · <b className="conflict-count">{conflicts.size}</b> conflicts</> : null}</span><button className="publish-button compact-publish" onClick={publish} disabled={!drafts || publishing}><Send size={15}/><span>{publishing ? "Publishing…" : drafts ? `Publish (${drafts})` : "Published"}</span></button></div>
    </section>
...
function Inventory({ products, setProducts, onNewProduct, onEditProduct, onStockCount, adjustments, setAdjustments, notify }: { products: Product[]; setProducts: React.Dispatch<React.SetStateAction<Product[]>>; onNewProduct: () => void; onEditProduct: (product:Product)=>void; onStockCount:()=>void; adjustments:StockAdjustment[]; setAdjustments:React.Dispatch<React.SetStateAction<StockAdjustment[]>>; notify: (s: string) => void }) {
  ...
  function adjust(product:Product,delta:number){ const reason=window.prompt(`Reason for ${delta>0?"adding":"removing"} ${Math.abs(delta)} ${product.unit}:`,delta>0?"Delivery / manual correction":"Waste / manual correction"); if(!reason)return; setProducts(cur=>cur.map(p=>p.id===product.id?{...p,stock:Math.max(0,p.stock+delta)}:p)); setAdjustments(cur=>[{id:crypto.randomUUID(),productId:product.id,productName:product.name,delta,reason,createdAt:new Date().toLocaleString("en-GB")},...cur].slice(0,30)); notify("Stock adjustment saved"); }
  return <>
...
function Orders({ products, setProducts, onNewOrder, notify }: { products:Product[]; setProducts:React.Dispatch<React.SetStateAction<Product[]>>; onNewOrder: () => void; notify: (s: string) => void }) {
  ...
  return <><PageHeader title="Purchase orders" subtitle="Generate suggested orders, receive deliveries and resolve invoice discrepancies." action={<button className="primary" onClick={onNewOrder}><Plus size={18} /> New order</button>} />
    ...
    <section className="panel table-panel"><div className="table-toolbar"><div className="search-field"><Search size={17} /><input placeholder="Search orders" /></div><button className="filter-button">All statuses <ChevronDown size={15} /></button></div><div className="data-table orders-table"><div className="table-row table-head"><span>Order</span><span>Supplier</span><span>Items</span><span>Delivery</span><span>Amount</span><span>Status</span></div>{orders.map((order) => <div className="table-row order-row" key={order.id}><span><b>{order.id}</b></span><span>{order.supplier}</span><span>{order.items}</span><span>{order.delivery}</span><span><b>{money(order.amount)}</b></span><span><i className={`status status-${received.includes(order.id)?"delivered":order.status.toLowerCase()}`}>{received.includes(order.id)?"Received":order.status}</i>{order.status!=="Delivered"&&!received.includes(order.id)&&<button className="icon-button" onClick={()=>receive(order.id)} title="Receive order"><CheckCircle2 size={16}/></button>}</span></div>)}</div></section>
  </>
}
