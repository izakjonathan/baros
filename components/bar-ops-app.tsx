...
function Schedule({ shifts, setShifts, employees, onNewShift, onEditShift, notify, currentWeekOffset, setCurrentWeekOffset, devMode, selectedLocationId, persist }: { shifts: Shift[]; setShifts: React.Dispatch<React.SetStateAction<Shift[]>>; employees: Employee[]; onNewShift: (date?: string) => void; onEditShift: (shift: Shift) => void; notify: (s: string) => void; currentWeekOffset: number; setCurrentWeekOffset: React.Dispatch<React.SetStateAction<number>>; devMode: boolean; selectedLocationId: string; persist: (path:string, options:RequestInit) => Promise<any> }) {
  ...
  return <div className={scheduleStyles.workspace}>
    ...
    <section className={`schedule-toolbar compact-schedule-toolbar ${scheduleStyles.toolbar} ${viewMode === "custom" ? "has-custom-range" : ""}`}>
      <div className={`period-controls ${scheduleStyles.periodControls}`}><button onClick={() => movePeriod(-1)} aria-label={`Previous ${viewMode}`}><ChevronLeft size={17}/></button><strong>{viewMode === "week" ? weekLabel : rangeLabel}</strong><button onClick={() => movePeriod(1)} aria-label={`Next ${viewMode}`}><ChevronRight size={17}/></button></div>
    ...
    <div className={`legend compact-legend ${scheduleStyles.legend}`}><span><i className="manager"/> Manager</span><span><i className="bartender"/> Bartender</span><span><i className="floor"/> Floor</span><span><i className="draft"/> Draft</span></div>
...
function Attendance({ employees, shifts, entries, setEntries, notify, onEdit, devMode, persist }: { employees: Employee[]; shifts: Shift[]; entries: TimeEntry[]; setEntries: React.Dispatch<React.SetStateAction<TimeEntry[]>>; notify:(s:string)=>void; onEdit:(entry:TimeEntry)=>void; devMode:boolean; persist:(path:string,options:RequestInit)=>Promise<any> }) {
  ...
  <section className="panel table-panel attendance-results"><PanelTitle title="Timesheets" subtitle="Review, correct and approve each record."/><div className="data-table attendance-table"><div className="table-row table-head"><span>Employee</span><span>Date</span><span>Clocked</span><span>Break</span><span>Variance</span><span>Status & actions</span></div>{visible.map(e=>{const actual=e.clockOut?workedHours(e):0;const variance=actual-e.scheduledHours;const exception=e.status!=="Running"&&(Math.abs(variance)>=.5||e.breakMinutes===0||e.edited);return <div className={`table-row ${exception?"exception-row":""}`} key={e.id}><span data-label="Employee"><b>{e.employee}</b>{e.edited&&<small>Manager corrected</small>}</span><span data-label="Date">{e.date}</span><span data-label="Clocked">{e.clockIn}–{e.clockOut||"Now"}<small>{e.clockOut?actual.toFixed(2)+"h":"Running"}</small></span><span data-label="Break">{e.breakMinutes} min</span><span data-label="Variance" className={Math.abs(variance)>=.5?"variance-alert":""}>{e.clockOut?`${variance>=0?"+":""}${variance.toFixed(2)}h`:"—"}</span><span data-label="Status" className="timesheet-actions"><i className={`status status-${e.status.toLowerCase()}`}>{e.status}</i>{e.status==="Running"&&<button type="button" className="approve-mini" onClick={()=>void toggleBreak(e)}>{e.onBreak?"End break":"Start break"}</button>}{e.status==="Pending"&&<><button title="Edit" onClick={()=>onEdit(e)}><Pencil size={14}/></button><button title="Reject" onClick={()=>rejectTimesheet(e.id)}><Ban size={14}/></button><button className="approve-mini" onClick={()=>approveTimesheet(e.id)}>Approve</button></>}{e.status==="Approved"&&<button title="Reopen" onClick={()=>reopenTimesheet(e.id)}><RotateCcw size={14}/></button>}{e.status==="Rejected"&&<button title="Return to review" onClick={()=>reopenTimesheet(e.id)}><RotateCcw size={14}/></button>}</span></div>})}{!visible.length&&<div className="attendance-empty"><strong>No timesheets found</strong><span>Change the employee, status or date filters to widen the results.</span></div>}</div></section>
  <section className="hours-by-employee"><PanelTitle title="Payroll export preview" subtitle="Approved hours by employee."/><div className="team-grid">{employees.map(emp=>{const scheduledEmp=visibleShifts.filter(s=>s.employee===emp.name).reduce((n,s)=>n+hoursBetween(s.start,s.end),0);const approvedEntries=approved.filter(e=>e.employee===emp.name);const workedEmp=approvedEntries.reduce((n,e)=>n+workedHours(e),0);return <article className="team-card payroll-preview-card" key={emp.name}><header><div className="avatar large">{emp.initials}</div><div><h2>{emp.name}</h2><p>{emp.role}</p></div></header><div className="hours-compare"><span>Scheduled<b>{scheduledEmp.toFixed(1)}h</b></span><span>Approved export<b>{workedEmp.toFixed(2)}h</b></span></div><small className="export-count">{approvedEntries.length} approved timesheet{approvedEntries.length===1?"":"s"}</small></article>})}</div></section>
...
function Inventory({ products, setProducts, onNewProduct, onEditProduct, onStockCount, adjustments, setAdjustments, notify, devMode, selectedLocationId, persist }: { products: Product[]; setProducts: React.Dispatch<React.SetStateAction<Product[]>>; onNewProduct: () => void; onEditProduct: (product:Product)=>void; onStockCount:()=>void; adjustments:StockAdjustment[]; setAdjustments:React.Dispatch<React.SetStateAction<StockAdjustment[]>>; notify: (s: string) => void; devMode:boolean; selectedLocationId:string; persist:(path:string,options:RequestInit)=>Promise<any> }) {
  ...
  return <>
    ...
    <section className="panel table-panel"><div className="table-toolbar inventory-toolbar"><div className="search-field"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search inventory" aria-label="Search inventory" /></div><select value={category} onChange={e=>setCategory(e.target.value)} aria-label="Filter inventory category">{categories.map(c=><option key={c}>{c}</option>)}</select><button className={`filter-button ${onlyLow ? "selected" : ""}`} aria-pressed={onlyLow} onClick={() => setOnlyLow(!onlyLow)}><AlertTriangle size={16} /> Below par</button>{(query||category!=="All categories"||onlyLow)&&<button type="button" className="secondary compact" onClick={()=>{setQuery("");setCategory("All categories");setOnlyLow(false)}}>Clear</button>}<span className="result-count" role="status" aria-live="polite">{filtered.length} product{filtered.length===1?"":"s"}</span></div>
...
function DailyOperations({tasks,setTasks,logs,setLogs,notify,devMode,locationId}:{tasks:OpsTask[];setTasks:React.Dispatch<React.SetStateAction<OpsTask[]>>;logs:LogEntry[];setLogs:React.Dispatch<React.SetStateAction<LogEntry[]>>;notify:(s:string)=>void;devMode:boolean;locationId:string}){
 ...
 return <><PageHeader title="Daily operations" subtitle="Opening, closing, handovers and maintenance in one live manager workspace." action={<div className="header-actions"><input type="date" value={serviceDate} max={new Date().toISOString().slice(0,10)} onChange={e=>setServiceDate(e.target.value)} aria-label="Checklist date"/><span className="connection-pill dev">{complete}/{tasks.length} complete</span></div>}/>
 ...
 <div className="operations-layout"><section className="panel ops-checklist"><PanelTitle title="Today’s checklist" subtitle="Completion is shared with every manager at this location."/>{isToday&&<div className="checklist-presets"><button type="button" className="secondary" onClick={()=>addPreset("Opening")}><Plus size={15}/>Opening checklist</button><button type="button" className="secondary" onClick={()=>addPreset("Closing")}><Plus size={15}/>Closing checklist</button></div>}<div className="task-list">{tasks.map(t=><article key={t.id} className={t.done?"task-done":""}><button type="button" className="task-check" disabled={!isToday||savingId===t.id} onClick={()=>toggleTask(t)}>{t.done?<Check size={16}/>:null}</button><span><b>{t.title}</b><small>{t.type} · {t.owner} · {t.due}</small></span>{isToday&&<span className="row-actions"><button type="button" className="icon-button" onClick={()=>editTaskMeta(t)} aria-label={`Edit responsibility for ${t.title}`}><Pencil size={15}/></button><button type="button" className="icon-button" onClick={()=>removeTask(t)} aria-label={`Remove ${t.title}`}><Trash2 size={15}/></button></span>}</article>)}</div>{isToday&&<div className="inline-create"><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Add an operational task"/><select value={type} onChange={e=>setType(e.target.value as OpsTask["type"])}><option>Task</option><option>Opening</option><option>Closing</option><option>Maintenance</option></select><button type="button" className="primary" onClick={addTask}><Plus size={16}/>Add</button></div>}</section>
