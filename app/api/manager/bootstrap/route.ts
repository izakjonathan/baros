...
export async function GET(request: Request) {
  try {
    ...
    const [employees, shifts, products, orders, timesheets, alerts, exports, templates, forecasts] = await Promise.all([
      ...
      db()`select t.*,e.first_name||' '||e.last_name employee_name,exists(select 1 from time_breaks b where b.timesheet_id=t.id and b.ended_at is null) on_break,(select b.started_at from time_breaks b where b.timesheet_id=t.id and b.ended_at is null order by b.started_at desc limit 1) open_break_started_at from timesheets t join employees e on e.id=t.employee_id where t.organization_id=${user.organizationId} and (${selectedLocationId}::uuid is null or t.location_id=${selectedLocationId}) order by t.work_date desc limit 500`,
