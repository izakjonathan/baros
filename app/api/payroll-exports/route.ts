...
type PayrollRow={payroll_id:string|null;employee:string;email:string|null;phone:string|null;role:string|null;salary_code:string|null;cost_centre:string|null;timesheets:number;minutes:number;ids:string[]|null};
...
export async function POST(req:Request){
 try{
  ...
  const result=await db().begin(async sql=>{
   ...
   const body=rows.map((r)=>[r.payroll_id,r.employee,r.email,r.phone,r.role,r.salary_code,r.cost_centre,period.starts_on,period.ends_on,r.timesheets,(r.minutes/60).toFixed(2)]);
   const content=[header,...body].map(r=>r.map(csv).join(',')).join('\r\n');const hash=createHash('sha256').update(content).digest('hex');const name=`payroll-${period.starts_on}-${period.ends_on}.csv`;const ids=rows.flatMap((r)=>r.ids||[]);const minutes=rows.reduce((n,r)=>n+Number(r.minutes),0);
