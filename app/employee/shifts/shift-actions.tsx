...
export function ClaimShiftButton({shiftId}:{shiftId:string}){
  ...
  return <div><button type="button" className="portal-action primary-action compact shift-card-action" disabled={state==='saving'||state==='done'} onClick={claim}>{state==='done'?'Request sent':state==='saving'?'Sending…':state==='error'?'Try again':'Request shift'}</button>{error&&<small className="form-error" role="alert">{error}</small>}</div>
}
