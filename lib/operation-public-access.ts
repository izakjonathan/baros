...
export async function loadPublicOperationState(access: PublicOperationAccess, date: string): Promise<OperationModuleState> {
  const [articles, tasks, needs] = await Promise.all([
    ...
    db()<Array<Record<string, unknown>>>`
      select id,title,note,status,created_at,quantity,supplier,priority,needed_by,ordered_at
      from operation_needs where organization_id=${access.organizationId}
      order by (status='NEEDED') desc,priority desc,needed_by nulls last,created_at desc limit 150`,
  ]);
  ...
  const mappedNeeds = needs.map((need): OperationNeed => ({ id: String(need.id), title: String(need.title), note: need.note == null ? null : String(need.note), status: String(need.status) === "ORDERED" ? "ORDERED" : "NEEDED", createdAt: String(need.created_at), quantity: need.quantity == null ? null : String(need.quantity), unit: null, supplier: need.supplier == null ? null : String(need.supplier), priority: String(need.priority) as OperationDailyTask["priority"], neededBy: need.needed_by == null ? null : String(need.needed_by), orderedAt: need.ordered_at == null ? null : String(need.ordered_at), orderedByName: null }));
  return { ...defaultOperationState, userRole: "EMPLOYEE", canManageContent: false, canManageTasks: false, storageStatus: "ready", today: date, handbook: articles.filter(article => article.kind === "HANDBOOK").map(mapOperationArticle), news: articles.filter(article => article.kind === "NEWS").map(mapOperationArticle), dailyTasks, assignees: [], taskTemplates: [], needs: mappedNeeds, metrics: { completionRate: dailyTasks.length ? Math.round(dailyTasks.filter(task => task.completed).length / dailyTasks.length * 100) : 100, completedCount: dailyTasks.filter(task => task.completed).length, dueCount: dailyTasks.length, overdueCount: 0, openNeedsCount: mappedNeeds.filter(need => need.status === "NEEDED").length, overdueNeedsCount: mappedNeeds.filter(need => need.status === "NEEDED" && need.neededBy && need.neededBy < date).length } };
}
