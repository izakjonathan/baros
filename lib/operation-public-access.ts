  ...
  const [articles, tasks, needs] = await Promise.all([
    ...
    db()<Array<Record<string, unknown>>>`
      ...
      from operation_needs where organization_id=${access.organizationId}
        and status='NEEDED'
      order by reminder_type,created_at desc limit 150`,
  ]);
