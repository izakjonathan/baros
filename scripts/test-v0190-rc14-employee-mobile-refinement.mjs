...
const rc = Number(pkg.version.match(/-rc\.(\d+)$/)?.[1] || 0);
if (rc < 17) {
  ...
  assert.match(css,/\.shift-action-row \{ display: flex; flex-wrap: wrap;/,'newer releases preserve compact mobile actions with safe wrapping');
  assert.match(css,/\.shift-card-action[^}]*white-space: nowrap;/,'newer card action labels remain single-line');
}
