...
assert.ok(mono.length < 44000, `mono-components.css should remain structurally consolidated; got ${mono.length} bytes`);
assert.doesNotMatch(globals,/\.schedule-summary\b/,"dead legacy schedule-summary selector must not return");
assert.doesNotMatch(mono,/\.settings-layout\b/,"dead legacy settings-layout selector must not return");
assert.doesNotMatch(mono,/\.table-head\b/,"dead legacy table-head selector must not return");
