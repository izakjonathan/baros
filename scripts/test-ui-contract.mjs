...
const checks = [
  ...
  ["Operation owner editor uses Quill rich text with Delta persistence", operationModule.includes('await import("quill")') && operationModule.includes("type DraftArticle = { id?: string; kind: OperationArticleKind; category: string; title: string; description: string; delta: OperationRichTextDelta }") && operationModule.includes('{ type: "richText", delta: cleanRichTextDelta(draft.delta) }') && operationModule.includes("function applyStyle(header: 1 | 2 | false)") && operationModule.includes("Heading</button><button") && operationModule.includes("Subheading</button><button") && operationModule.includes("Body</button>") && operationModule.includes('if (normalizeHeader(line.attributes?.header) === 1) return <h1>{content}</h1>') && operationModule.includes('if (normalizeHeader(line.attributes?.header) === 2) return <h2>{content}</h2>') && operationModule.includes('className="ql-image"') && operationModule.includes('fetch("/api/operation-images"') && operationModule.includes("function RichTextArticle") && operationModule.includes('article.description || "No article content has been added yet."') && operation.includes(".articleEditor{position:fixed;") && operation.includes(".quillToolbar{position:fixed") && operation.includes("--editor-keyboard-offset") && operation.includes(".styleMenu{position:absolute") && operation.includes(".quillSurface")],
