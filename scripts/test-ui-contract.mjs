...
const checks = [
  ...
  ["standalone Operation module owns its separate shell", read("app/operation/page.tsx").includes("<OperationModule") && read("features/operation/OperationModule.tsx").includes("className={styles.operationShell}") && /\.operationShell\{[^}]*min-height:100dvh/.test(operation) && operation.includes(".reader{position:fixed;inset:0")],
  ...
  ["Operation reader has only the bottom outlined close control", !operationModule.includes("<button type=\"button\" onClick={close}>Close</button>") && operation.includes(".closeCircle,.readerBackCircle{width:3.5rem;height:3.5rem;display:grid!important;place-items:center") && operation.includes(".handbookCloseCircle{background:var(--op-canvas)!important}")],
  ...
  ["Operation owner editor uses constrained Tiptap rich text with legacy Delta reading", operationModule.includes('from "@tiptap/react"') && operationModule.includes("type DraftArticle = { id?: string; kind: OperationArticleKind; category: string; title: string; description: string; document: OperationTiptapDocument }") && operationModule.includes('{ type: "tiptap", document: draft.document }') && operationModule.includes('StarterKit.configure({ heading: { levels: [2, 3] } })') && operationModule.includes('applyStyle("heading")') && operationModule.includes('applyStyle("subheading")') && operationModule.includes('applyStyle("body")') && operationModule.includes('fetch("/api/operation-images"') && operationModule.includes("function RichTextArticle") && operationModule.includes("function TiptapArticle") && operationModule.includes('article.description || "No article content has been added yet."') && operation.includes(".articleEditorFrame{position:fixed;top:var(--editor-vv-top,0px)") && operation.includes(".tiptapToolbar{position:fixed") && operation.includes("--editor-vv-height") && operation.includes(".styleMenu{position:absolute") && operation.includes(".tiptapSurface")],
  ["Operation storage migration state prevents silent failed saves", operationModule.includes('const migrationMessage = "Operation storage is not ready yet. Run the database migration action, then reload this page."') && operationModule.includes('state.storageStatus === "migration-required"') && operationModule.includes("function StorageNotice") && operationModule.includes('if (!response.ok) { setNeedMessage(await responseMessage(response, "Could not add reminder.")); return; }') && operation.includes(".storageNotice{")],
  ...
  ["public Operation route is a shared staff link with employee-level Operations access", publicOperationPage.includes("operationApiUrl={apiUrl}") && publicOperationRoute.includes("assignment_scope='EVERYONE'") && publicOperationRoute.includes("PUBLIC_OPERATION_TASK_UPDATED") && publicOperationRoute.includes("PUBLIC_OPERATION_REMINDER_UPDATED") && publicOperationRoute.includes("PUBLIC_OPERATION_CASH_COUNT_CREATED") && operationModule.includes("operationApiUrl")],
