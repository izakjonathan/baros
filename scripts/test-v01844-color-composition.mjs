...
for (const [name, css] of [['team',team],['dashboard',dashboard],['schedule',schedule]]) {
  for (const color of ['#dfee4b','#f47add','#9561e6','#eb6746']) if(!css.includes(color)) fail(`${name} is missing ${color}`);
}
...
if(!schedule.includes('background:#f47add')||!schedule.includes('background:#eb6746')) fail('schedule color architecture missing');
