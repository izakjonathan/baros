...
const fail=(m)=>{console.error(`v0.18.4.4 regression: ${m}`);process.exit(1)};
if(pkg.version!=='0.18.4.4') fail('package version mismatch');
for (const [name, css] of [['team',team],['dashboard',dashboard],['schedule',schedule]]) {
  for (const color of ['#dfee4b','#f47add','#4e4ced','#feb34a']) if(!css.includes(color)) fail(`${name} is missing ${color}`);
}
...
if(!schedule.includes('background:#f47add')||!schedule.includes('background:#feb34a')) fail('schedule color architecture missing');
console.log('v0.18.4.4 color architecture regression passed');
