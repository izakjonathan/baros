...
const password=process.env.SEED_OWNER_PASSWORD||"ChangeMe123!", salt=randomBytes(16).toString('hex'), derived=await scrypt(password,salt,64), hash=`scrypt:${salt}:${derived.toString('hex')}`;
await sql.begin(async tx=>{
 ...
 const [user]=await tx`insert into users(email,name,password_hash) values(${process.env.SEED_OWNER_EMAIL||'owner@barops.local'},'Izak Hyllested',${hash}) on conflict(email) do update set name=excluded.name,password_hash=excluded.password_hash returning id`;
...
console.log(`Seed complete. Manager: ${process.env.SEED_OWNER_EMAIL||'owner@barops.local'} / ${password}. Employee: maya@barops.local / Employee123!`); await sql.end();
